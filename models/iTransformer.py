...
class Model(nn.Module):
    ...
    def interpolate(self, x):
        ...
        x = F.interpolate(x, size=self.interpolate_len,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.save_mid_tensor_for_PCA:
            ...
            torch.save(saved_tensor, os.path.join(
                self.save_dir_org, 'x_enc_' + str(self.count) + '.pth'))
        ...
        if self.save_mid_tensor_for_PCA:
            ...
            torch.save(saved_tensor, os.path.join(
                self.save_dir, 'enc_out_' + str(self.count) + '.pth'))
            self.count += 1
